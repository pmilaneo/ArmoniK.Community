...
Following KISS principle is always a good idea.

## APIs shloud have comments

Service, Messages, Enums and RPC methods should ne commented so that documentation can be generated.
...
The use of [Buf Linter](https://buf.build/docs/lint/overview) is suggested to validate that the proto files are properly writen.
...
It highly increase the difficulty of using a message when they are used in streams and/or responses.
...
Some systems does not support streaming such as [gRPC web](https://github.com/grpc/grpc-web) that does not support streaming from client to server.
...
RPC methods should have an error message that will be sent as response metadata when there is an error during the call.
